...
class AmorphGameController():
	def __init__(self):
		...
		#pointless, except to make bounce method work
		self.direction = None
...
class GreenSprite(pygame.sprite.Sprite):
	def __init__(self):
		...
		self.color = pygame.Color(random.randrange(50,150),random.randrange(50,150),random.randrange(100,200))
		pygame.gfxdraw.filled_circle(self.image, 40, 40, self.radius, self.color)
...
def main():
	...
	while(running):
		...
		for blob in pygame.sprite.spritecollide(game_control.player_group.sprite, game_control.enemy_group, False, collided=pygame.sprite.collide_circle):
			_bounce(blob, game_control.player_group.sprite)
